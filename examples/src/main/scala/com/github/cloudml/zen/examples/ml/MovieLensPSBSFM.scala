...
import com.github.cloudml.zen.ml.parameterserver.recommendation.BSFMRegression
...
object MovieLensPSBSFM extends Logging {
  ...
  def run(params: Params): Unit = {
    ...
    val conf = new SparkConf().setAppName(s"PSFM with $params")
    if (kryo) {
    ...
    val numFeatures = trainSet.first()._2.features.size
    ...
    val lfm = new BSFMRegression(trainSet.map(_._2), views, rank, stepSize,
      l2, batchSize, 1.0, eta)
    var iter = 0
