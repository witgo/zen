...
import com.github.cloudml.zen.ml.recommendation.MFALS._
 ...
 * (\sum_{i_m =1}^{I_m+1}z_{i_m}^{(m)}a_{i_m,j}^{(m)})
 */
private[ml] abstract class MFALS extends Serializable with Logging {
  ...
  protected def backward(q: VertexRDD[VD], iter: Int): (Long, Double, VertexRDD[VD]) = {
    ...
    val gradient = GraphImpl.fromExistingRDDs(multi, edges).aggregateMessages[VD](ctx => {
      ...
      val arr = ctx.dstAttr
      val viewId = featureId2viewId(featureId, views)
      val m = backwardInterval(rank, viewId, x, arr, arr.head)
      ctx.sendToSrc(m)
        ...
        case Some(grad) =>
          ...
          if (iter % views.length == viewId) {
            ...
            while (i < rank) {
              ...
              val h2 = grad(i + rank)
              val w = weight(i)
              weight(i) = he / (h2 + lambda)
              if (Utils.random.nextDouble() < 1e-12) {
...
class MFALSRegression(
  @transient _dataSet: Graph[VD, ED],
  val lambda: Double,
  val views: Array[Long],
  val rank: Int,
  val storageLevel: StorageLevel) extends MFALS {
  ...
  def this(
    ...
    this(initializeDataSet(input, views, rank, storageLevel), lambda, views, rank, storageLevel)
  }
  ...
  override protected def multiplier(q: VertexRDD[VD], iter: Int): (Long, Double, VertexRDD[VD]) = {
    val accCostSum = q.context.accumulator[Double](0.0, "costSum")
    ...
    val numSamples = multi.filter(t => t._2.length == rank * views.length + 1).map { t =>
      1L
    }.reduce(_ + _)
    (numSamples.toLong, accCostSum.value, multi)
  }
}
...
object MFALS {
  ...
  def trainRegression(
    ...
    val lfm = new MFALSRegression(data, lambda, views, rank, storageLevel)
