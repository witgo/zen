...
abstract class Gradient extends Serializable {
   ...
   */
  def compute(data: Vector, label: Vector, weights: Vector): (Vector, Double) = {
   ...
   * @return loss
   */
  def compute(data: Vector, label: Vector, weights: Vector, cumGradient: Vector): Double

  /**
   * Compute the gradient and loss given the iterator.
   *
   * @param iter Iterator for (data, label) pair
   * @param weights weights/coefficients corresponding to features
   * @param cumGradient the computed gradient will be added to this vector
   *
