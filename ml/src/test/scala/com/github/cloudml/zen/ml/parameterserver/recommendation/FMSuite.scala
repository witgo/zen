...
class FMSuite extends FunSuite with SharedSparkContext with Matchers {
  test("movieLens 1m (uid,mid) ") {
    ...
    val movieLens = sc.textFile(dataSetFile, 4).repartition(4).mapPartitions { iter =>
    ...
    val lfm = new FMRegression(trainSet, rank, stepSize, regParam, miniBatch, samplingFraction, eta)
    var iter = 0
